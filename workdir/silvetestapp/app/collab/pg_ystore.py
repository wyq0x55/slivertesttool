...
class PgYStore(BaseYStore):
    ...
    def _compact_locked(self) -> None:
        ...
        if len(rows) < 2:
            return
        # FIX: merge_updates takes a list[bytes], not *args.
        # The original `merge_updates(*[...])` unpacked the list as positional
        # arguments, which raises TypeError at runtime when the list has ≥2
        # elements because pycrdt expects exactly one list argument.
