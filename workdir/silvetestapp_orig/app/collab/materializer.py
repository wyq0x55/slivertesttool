...
not reliable. We therefore gate self-originated writes with an explicit flag.
...
class Materializer:
    ...
    @contextmanager
    def suppressed(self):
        """Mark ``Y.Doc`` writes made inside this block as self-originated so the
        observer ignores them (use when writing server data back into the doc)."""
        self._suppress = True
        try:
            yield
        finally:
