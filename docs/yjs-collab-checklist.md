...
- [ ] awareness（三项，按难度拆分）：
  ...
  - [x] **远端光标/选区自绘 overlay**（uuid→当前视图行列换算，设计 §6.1）。本端选区/单元格聚焦时经 `getActiveCell()`（FallbackGrid 由聚焦 `td` 反查列；Univer adapter 由单格选区记 `{id,col}`）→ `setLocalCursor(sheet,uuid,col)` 把**列**一并写入 awareness `cursor`。`editor.js::applyCollabPresence` 两种**互斥**模式（绝不同时出现）：①**精确光标 overlay** —— `grid.setRemoteCursors(cursors)` 优先；FallbackGrid 在 grid host 内叠一层 `pointer-events:none` 绝对定位 overlay，按行 `uuid→id`、`col→field_key` 定位目标 `td`，用**内容坐标**（`offsetLeft/Top/Width/Height`）画用户色描边框 + 名字 tag（随表滚动免重算，`render()` 后自动重绘，行被筛掉则跳过；免构建，`grid.js`/`lanmatrix.css` 直接生效）。②**退回行高亮** —— overlay 不可用/被拒时（Univer canvas 无稳定像素坐标 API，`adapter.ts::setRemoteCursors` 返回 `false`）自动改用 §6.1 第 2 项的行淡底（Univer 经 facade `setBackgroundColor`，**需 `npm run build`**）。Univer 用户仍向他人广播精确列，故 FallbackGrid 侧能看到其精确光标。
- [x] **Excel 导入走 `Y.Array` 批量事务**（一次大 transact，避免逐行抖动）。服务端导入落库后，`editor.js::resyncSheetFromDb` → `collab.js::reconcileFromDb` 把整表 upsert/删除**包在单个 `doc.transact` 内**（uuid keyed：命中则逐字段 diff 后 `set`，缺失则 `insert` 新 `Y.Map`，服务端已删的行按有效 `id` 删除），远端只收到一次事务、只重绘一次，无逐行抖动。
- [x] **断线重连、token 到期前静默续签、离线编辑缓冲后重放**。①断线重连：y-websocket 内建指数退避自动重连，`onStatus` 已把 connecting/connected/disconnected 映射到状态徽标（断开显示「离线编辑，重连中…」）。②静默续签：token 仅在握手时校验（开着的 socket 不受过期影响），但过期后的重连会被拒——新增 `_scheduleTokenRenewal`/`_renewToken`，在到期前 ~45s（`TOKEN_RENEW_SKEW_MS`，短 token 下限 20s）自动 `POST /collab-token` 拿新 token 并写回 `provider.params.token`，使后续每次（重）连都携带有效 token；拉取失败按 15s 重试，`_teardown` 清理定时器。`tokens.py` 陈旧的「续签待实现」注释已更新。③离线缓冲重放：Yjs `Y.Doc` 天然缓冲离线本地 update，重连时由 provider 自动 sync 回放（编辑器断线后保持在 Y.Doc 上离线编辑，恢复后自动合并）。纯前端、免构建（`collab.js`）。
- [ ] 大矩阵（数千行）性能压测：首次 sync 时延、内存、Univer 虚拟化在 CRDT 投影下仍生效。**（需真实环境跑：沙箱无 PG/pycrdt/浏览器，留给部署后实测。）**
- [x] **单元格级校验错误回报（某行非法不阻断他行物化）**。①**逐行隔离**：`items_service.materialize_sheet` 把每行的 create/update 包进独立 `db.session.begin_nested()`（SAVEPOINT）——任一行抛异常只回滚该行的 savepoint，其余行照常提交（此前整表单事务，一行崩全表不物化，正是用户遇到的物化崩溃类问题的根因加固）。②**非阻断校验**：每行先跑 `validation.validate_record(..., enforce_required=False)`（实时协同下行常处于半填状态，故不因“必填未填”报错，只拦硬错误：数字/日期/枚举非法），阻断级错误则**保留该行上一次的有效 DB 值**、不落库坏值。③**回报通道**：`materialize_sheet` 返回 `{...,failed,errors:{uuid:{cells,message}}}`；materializer 汇总为 `{uuid:{cells,message,sheet}}` 后经 `doc_model.write_row_errors` 写入 Y.Doc 顶层共享 `row_errors` Map（服务端单写者，每次 flush 重建快照→修好的行自动清除），在同一个 `suppressed()` 事务里随 id 回写一起提交。④**前端标红**：`collab.js` 观察 `row_errors` 经 `onRowErrors` 回调 → `editor.js::applyCellErrors` uuid→当前行 id → `grid.setCellErrors({id:{cells,message}})`；FallbackGrid 给对应 `td` 加 `.lm-cell-collab-error`（红框 + hover 提示，独立于本地保存用的 `.lm-cell-error`，免构建）；Univer adapter 经 facade `setBackgroundColor`+`setNote` 标红对应单元格（**需 `npm run build`**）。渲染后自动重绘（repaint 会清掉标记）。
